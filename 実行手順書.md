...
input.md（原稿：通常のテキスト形式）
    ↓ npm run convert:youyaku
youyaku.md（要約版：input.mdを短くしたもの）自動生成
...
## ステップ1: youyaku.md（要約版）の生成

### 1-1. input.md から youyaku.md への変換

以下のコマンドで、**input.md** から **youyaku.md** を自動生成します：

```bash
npm run convert:youyaku
```

このコマンドは、input.md を読み取り、要約版の youyaku.md を生成します。
**注意:** input.md は変更されず、そのまま残ります。

---

## ステップ2: movieyou.md（動画用台本）の生成

### 2-1. youyaku.md から movieyou.md への変換

以下のコマンドで、**youyaku.md** から **movieyou.md** を自動生成します：

```bash
npm run convert:movieyou
```

このコマンドは、youyaku.md を読み取り、**マークダウン形式**で movieyou.md を生成します：
...
### ayumi
ガイドラインが上限内に収まるなら、 毎回プロンプトへ全投入する方が精度は安定です。。
...
## ステップ3: TypeScript台本ファイルの生成

### 3-1. movieyou.md から transcripts/myvideo.tsx への変換
...
# 1. youyaku.md を生成（input.mdから要約版を作成）
npm run convert:youyaku

# 2. movieyou.md を生成（youyaku.mdから動画用台本を作成）
npm run convert:movieyou
...
# 5. 音声の長さを台本に反映（⚠️ 必須: スキップすると音声が重複します）
npm run update:audio-durations
...
**⚠️ 重要な注意事項:**
- **ステップ5 (`npm run update:audio-durations`) は必ず実行してください**
- このステップをスキップすると、`fromFramesMap`が生成されず、全セリフが同時に再生されて音声が重複します
- 音声ファイルを再生成した場合は、必ずステップ5を再実行してください
- input.mdは変更されません。要約版はyouyaku.mdに保存されます
...
2. **youyaku.md** で要約版を自動生成（input.mdは変更されない）
3. **movieyou.md** でマークダウン形式に自動変換（話者: ayumi）
4. **TypeScript台本** で動画設定を自動生成
5. **VOICEPEAK** で音声を自動生成
6. **Remotion** でプレビュー確認
7. **カスタムレイアウト** で背景画像・人物画像を設定（オプション）
